...
from sqlalchemy import Column, ForeignKey, Integer#, String#,Boolean, DateTime, func, ForeignKey
...
class CorpusLabel(Base):
    ...
    label_id = Column(Integer,ForeignKey('label.id'),nullable=False, unique=True)
    corpus_id = Column(Integer,ForeignKey('corpus.id'),nullable=False, unique=True)
    label = relationship("Label", back_populates="corpus_label")
    corpus = relationship("Corpus", back_populates="corpus_label")
